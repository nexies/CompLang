...
//                cout << "Cmd_new executed" << endl;
//                stream.check_container();
                return true;
            }
        }
//        cout << "Cmd_new didn't execute" << endl;
//        stream.check_container();
        return false;
    }
};
...
//                cout << "Cmd_in didn't execute. No variable" << endl;
//                stream.check_container();
                return false;
            }
            int n;
...
//            cout << "Cmd_in executed" << endl;
//            stream.check_container();
            return true;
        }
//        cout << "Cmd_in didn't execute" << endl;
//        stream.check_container();
        return false;
    }
};
...
//            stream.check_container();
        ...
        BitSeq var = stream.read(2);
        if(!vars->contains(var)){
//            cout << "Cmd_new didn't execute. No var" << endl;
//            stream.check_container();
            return false;
        }
        int n = (int)*(vars->at(var)->data().begin().byte);
        cout << n << endl;
//        cout << "Cmd_out executed" << endl;
//        stream.check_container();
        return true;
    }
};
...
//Проблема: BitSeq(bits, len) записывает биты задом наперед!
int main()
{
